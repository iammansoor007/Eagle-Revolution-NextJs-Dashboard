          ...
          <motion.div key={activeTab} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="space-y-16 pb-20">
              ...
              <div className="max-w-4xl grid grid-cols-12 gap-12">
                 <div className="col-span-7 space-y-10">
                    <div className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
                       <div className="space-y-3">
                          <label className="text-[10px] font-medium text-slate-400 uppercase tracking-widest">Global Site Title</label>
                          <input type="text" value={data.settings?.siteTitle || ""} onChange={(e) => updateNested(["settings", "siteTitle"], e.target.value)} className="w-full bg-slate-50 px-5 py-4 rounded-xl text-lg font-bold outline-none" />
                       </div>
                       <div className="space-y-3">
                          <label className="text-[10px] font-medium text-slate-400 uppercase tracking-widest">Metadata Template (e.g. %s | Brand)</label>
                          <input type="text" value={data.settings?.siteTemplate || ""} onChange={(e) => updateNested(["settings", "siteTemplate"], e.target.value)} className="w-full bg-slate-50 px-5 py-3.5 rounded-xl text-sm font-mono outline-none" />
                       </div>
                    </div>
                 </div>
              ...
              <div className="space-y-12">
                 <div className="max-w-3xl bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
                    <div className="grid grid-cols-2 gap-8">
                       <div className="space-y-3">
                          <label className="text-[10px] font-medium text-slate-400 uppercase tracking-widest">Navbar CTA Text</label>
                          <input type="text" value={data.navbar?.ctaText || ""} onChange={(e) => updateNested(["navbar", "ctaText"], e.target.value)} className="w-full bg-slate-50 px-5 py-3 rounded-xl text-sm font-bold outline-none" />
                       </div>
                       <div className="space-y-3">
                          <label className="text-[10px] font-medium text-slate-400 uppercase tracking-widest">Navbar CTA Link</label>
                          <input type="text" value={data.navbar?.ctaLink || ""} onChange={(e) => updateNested(["navbar", "ctaLink"], e.target.value)} className="w-full bg-slate-50 px-5 py-3 rounded-xl text-sm outline-none" />
                       </div>
                    </div>
                 </div>

                 <div className="space-y-6">
                    <label className="text-[10px] font-medium text-slate-400 uppercase tracking-widest">Main Menu Links</label>
                    <div className="grid grid-cols-1 gap-4 max-w-4xl">
                       {(data.navbar?.companyLinks || []).map((link: any, i: number) => (
                         <div key={i} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-6 group">
                            <div className="flex items-center gap-6">
                               <div className="flex-1 grid grid-cols-2 gap-4">
                                  ...
                                  }} className="bg-slate-50 px-4 py-2 rounded-xl text-xs font-bold outline-none" placeholder="Link Label" />
                                  ...
                                  }} className="bg-slate-50 px-4 py-2 rounded-xl text-xs outline-none" placeholder="URL Path" />
                               </div>
                               <button onClick={() => {
                            ...
                            <div className="pl-10 border-l-2 border-slate-50 space-y-3">
                               {(link.subLinks || []).map((sub: any, j: number) => (
                                 <div key={j} className="flex items-center gap-4">
                                    <div className="flex-1 grid grid-cols-2 gap-3">
                                       ...
                                       }} className="bg-slate-50/50 px-3 py-1.5 rounded-lg text-[10px] font-medium outline-none" placeholder="Sub-link Label" />
                                       ...
                                       }} className="bg-slate-50/50 px-3 py-1.5 rounded-lg text-[10px] outline-none" placeholder="Sub-link Path" />
                                    </div>
                                    <button onClick={() => {
                                    ...
                                    }} className="text-slate-200 hover:text-red-400"><X className="w-3.5 h-3.5" /></button>
                                 </div>
                               ))}
                               <button onClick={() => {
                               ...
                               }} className="text-[9px] font-bold text-primary uppercase tracking-widest">+ Add Sub-Menu Link</button>
                            </div>
                         </div>
                       ))}
                       <button onClick={() => updateNested(["navbar", "companyLinks"], [...(data.navbar?.companyLinks || []), { label: "New Page", href: "/", icon: "Globe", subLinks: [] }])} className="border-2 border-dashed border-slate-200 rounded-2xl py-8 flex flex-col items-center justify-center gap-2 text-slate-300 hover:text-primary transition-all">
                          <Plus className="w-6 h-6" />
                          <span className="text-[10px] font-bold uppercase tracking-widest">Insert Header Menu Item</span>
                       </button>
                    </div>
                 </div>
              </div>
            ...
            {activeTab === "footer" && (
              <div className="grid grid-cols-12 gap-12">
                 <div className="col-span-6 space-y-12">
                    <div className="space-y-6">
                       <label className="text-[10px] font-medium text-slate-400 uppercase tracking-widest">Footer Identity</label>
                       <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm space-y-6">
                          <div className="space-y-2">
                             <label className="text-[9px] font-bold text-slate-300 uppercase">Description</label>
                             <textarea value={data.footer?.company?.description || ""} onChange={(e) => updateNested(["footer", "company", "description"], e.target.value)} rows={3} className="w-full bg-slate-50 px-4 py-3 rounded-xl text-xs text-slate-500 outline-none" />
                          </div>
                    ...
                    <div className="space-y-6">
                       <label className="text-[10px] font-medium text-slate-400 uppercase tracking-widest">Marquee Text Bar</label>
                       <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm space-y-6">
                          {(data.footer?.marquee?.texts || []).map((text: string, idx: number) => (
                            <input key={idx} type="text" value={text} onChange={(e) => {
                               const newT = [...data.footer.marquee.texts]; newT[idx] = e.target.value; updateNested(["footer", "marquee", "texts"], newT);
                            }} className="w-full bg-slate-50 px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest outline-none" />
                          ))}
                       </div>
                    </div>
                 </div>
                 ...
                 <div className="col-span-6 space-y-12">
                    <div className="space-y-6">
                       <label className="text-[10px] font-medium text-slate-400 uppercase tracking-widest">Bottom Bar & Links</label>
                       <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm space-y-6">
                          <div className="space-y-2">
                             <label className="text-[9px] font-bold text-slate-300 uppercase">Copyright Text</label>
                             <input type="text" value={data.footer?.bottom?.copyright || ""} onChange={(e) => updateNested(["footer", "bottom", "copyright"], e.target.value)} className="w-full bg-slate-50 px-4 py-2 rounded-xl text-xs outline-none" />
                          </div>
                          <div className="space-y-2">
                             <label className="text-[9px] font-bold text-slate-300 uppercase">Secondary Bar Links</label>
                             <div className="grid grid-cols-1 gap-2">
                                {(data.footer?.bottom?.links || []).map((link: any, idx: number) => (
                                  <div key={idx} className="flex gap-2">
                                     ...
                                     }} className="flex-1 bg-slate-50 px-3 py-1.5 rounded-lg text-[10px] outline-none" />
                                     <input type="text" value={link.href} onChange={(e) => {
                                        const newL = [...data.footer.bottom.links]; newL[idx].href = e.target.value; updateNested(["footer", "bottom", "links"], newL);
                                     }} className="flex-1 bg-slate-50 px-3 py-1.5 rounded-lg text-[10px] outline-none" />
                                  </div>
                                ))}
                                <button onClick={() => updateNested(["footer", "bottom", "links"], [...(data.footer?.bottom?.links || []), { label: "Legal", href: "/legal" }])} className="text-[9px] font-bold text-primary uppercase">+ Add Link</button>
                             </div>
                          </div>
                       </div>
                    </div>
                    ...
                    <div className="space-y-6">
                       <label className="text-[10px] font-medium text-slate-400 uppercase tracking-widest">Newsletter Config</label>
                       <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm grid grid-cols-2 gap-4">
                          <div className="space-y-2">
                             <label className="text-[9px] font-bold text-slate-300 uppercase">Placeholder</label>
                             <input type="text" value={data.footer?.newsletter?.placeholder || ""} onChange={(e) => updateNested(["footer", "newsletter", "placeholder"], e.target.value)} className="w-full bg-slate-50 px-4 py-2 rounded-xl text-xs outline-none" />
                          </div>
                          <div className="space-y-2">
                             <label className="text-[9px] font-bold text-slate-300 uppercase">Btn Text</label>
                             <input type="text" value={data.footer?.newsletter?.buttonText || ""} onChange={(e) => updateNested(["footer", "newsletter", "buttonText"], e.target.value)} className="w-full bg-slate-50 px-4 py-2 rounded-xl text-xs outline-none" />
                          </div>
                       </div>
                    </div>
                 </div>
            ...
            {activeTab === "vitals" && (
              <div className="grid grid-cols-12 gap-12">
                 <div className="col-span-6 space-y-12">
                    <div className="space-y-6">
                       <label className="text-[10px] font-medium text-slate-400 uppercase tracking-widest">Opening Hours</label>
                       <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
                          {Object.entries(data.hours || {}).map(([day, val]: [string, any]) => (
                            <div key={day} className="flex items-center justify-between">
                               ...
                               <input type="text" value={val} onChange={(e) => updateNested(["hours", day], e.target.value)} className="w-40 bg-slate-50 px-3 py-1.5 rounded-lg text-xs font-bold text-right outline-none" />
                            </div>
                          ))}
                       </div>
                    </div>
                 ...
                 <div className="col-span-6 space-y-12">
                    <div className="space-y-6">
                       <label className="text-[10px] font-medium text-slate-400 uppercase tracking-widest">Service Areas</label>
                       <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
                          <textarea value={data.footer?.contact?.areas || ""} onChange={(e) => updateNested(["footer", "contact", "areas"], e.target.value)} rows={5} className="w-full bg-slate-50 px-5 py-4 rounded-2xl text-sm text-slate-500 outline-none leading-relaxed" placeholder="St. Louis, MO and surrounding areas..." />
                       </div>
                    </div>
                 </div>
              </div>
